        ...
        <div class="col-lg-12">
            <div class="card">
                <div class="customer-card card-head mx-5 mt-5 mb-3" style="margin-top:-10px;">
                    <div class="area-h3">
                        <h2>Category wise commission</h2> </div>
                    <div class="print">
                        <a href="{{route('showMerchantCommissionStore')}}" class="btn btn-primary pdf">Category wise commission List</a>
                    </div>

                </div>
                <div class="card-body">
                    <form action="{{ route('commission.calculate')}}" method="POST" enctype="multipart/form-data">
                        @csrf
                        <input type="hidden" name="qrCode_id" value="{{ $qrCode->id }}">
                        <div class="form-group">
                            <label for="merchant_id">Category Name</label>
                            <select name="offer_id" id="merchant_id" class="form-control my-2" {{ $someVariable ? 'disabled' : '' }}>
                                @foreach($offer as $values)
                                    <option value="{{ $values->id }}" {{ $values->id == $qrCode->offer_id ? 'selected' : '' }}>
                        ...
                        <div class="form-group">
                                ...
                                <div class="form-group">
                                    ...
                                    <select id="minimum_amount" name="minimum_amount" class="form-control my-2">
