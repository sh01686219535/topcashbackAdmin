        ...
        <div class="col-lg-12">
            ...
            <div class="card">
                ...
                <div class="customer-card mb-3" style="margin-top:-10px;">
						<div class="area-h3">
							<h2>Merchant Rate Off Place List</h2> </div>
						<div class="print"> 
                        ...
                        <a href="{{route('add.merchant.rate')}}" class="btn btn-primary" title="Add Role"><i class="fa-solid fa-plus"></i> Create Merchant Rate Off Place</a>
                    ...
                    <table class="table table-hover table-borderd" id="example">
                        ...
                        <tbody>
                           ...
                           $i = 1
                           @endphp
                           @foreach($merchantAdd as $item)
                           <tr>
                            <td>{{ $i++ }}</td>
                            <td>{{$item->rateOffPrice->placeName}}</td>
                            <td>{{$item->rateOffPrice->ratePlace}}</td>
                            <td>{{$item->merchant->areas->areaName}}</td>
                            <td>{{$item->expiryDate}}</td> 
                            <td>{{\carbon\carbon::create($item->created_at)->format('d-M-y')}}</td>
                            <td>
                                <a href="{{route('edit.merchant.rate',$item->id)}}" title="Edit" class="btn btn-primary"><i class="fa-regular fa-pen-to-square"></i></a>
                                <a href="{{route('delete.merchant.rate',$item->id)}}" title="Delete" class="btn btn-danger" id="delete"><i class="fa-solid fa-trash"></i></a>
                            </td>
                           </tr>
                           @endforeach
                        </tbody>
